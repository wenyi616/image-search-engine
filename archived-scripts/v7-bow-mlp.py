...
##########################################################
mlp = MLPRegressor()
mlp.fit(training_vectors, training_labels)
mlp_pred = mlp.predict(testing_vectors)

print(mlp_pred.shape)
print(mlp_pred)
print("\n**** Done MLP Model ****")   

np.savetxt("./output/mlp_pred.csv",mlp_pred, delimiter=",")
print("\n**** Done saving output ****")   
...
submission_df.to_csv ("./output/mlp_output.csv", index = None, header=True)
